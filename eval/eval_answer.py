...
用法：
    python -m eval.eval_answer                       # 单跳模式，评估全部题目
    python -m eval.eval_answer --mode agent          # 多跳 Agent 模式
    python -m eval.eval_answer --limit 3             # 只评前 3 题（省配额）
    python -m eval.eval_answer --save out.json       # 同时落盘逐题明细
    python -m eval.eval_answer --mode agent --save agent_ans.json  # Agent 版落盘对比
"""
import sys
import json
import argparse
...
def evaluate(limit: int = None, save: str = None, mode: str = "single"):
    ...
    rag = LegalRAG()
    # agent 模式：构建多跳 Agent（复用同一 rag 实例，共享 VectorStore）
    agent = None
    if mode == "agent":
    ...
    for i, case in enumerate(cases, 1):
        print(f"[{i}/{len(cases)}] [{mode}] {case['question']}")
        # 跑完整链路（非流式，便于评估）：agent 走多跳，single 走单跳
        if mode == "agent":
        ...
        hops = result.get("hops", 1)  # agent 返回跳数，single 默认 1
        print(f"     acc={scores['accuracy']} ground={scores['grounding']} "
              f"cite={scores['citation']} clarity={scores['clarity']} "
              f"hops={hops}  ▸ {scores['comment']}")
        rows.append({
    ...
    n = len(cases)
    print(f"\n{'='*60}")
    print(f"答案质量评估 · {mode}  (题目数={n}, 裁判={config.LLM_PROVIDERS[0]['model']})")
    print(f"{'='*60}")
    for d in DIMENSIONS:
    ...
    if save:
        ...
        with open(out, "w", encoding="utf-8") as f:
            json.dump({"mode": mode,
                       "summary": {d: round(totals[d]/n, 2) for d in DIMENSIONS},
...
if __name__ == "__main__":
    ...
    ap.add_argument("--mode", choices=["single", "agent"], default="single",
                    help="single=单跳 RAG；agent=多跳 Agent")
    ap.add_argument("--limit", type=int, default=None, help="只评前 N 题（省配额）")
    ap.add_argument("--save", type=str, default=None, help="逐题明细落盘路径")
    args = ap.parse_args()
    evaluate(args.limit, args.save, args.mode)
